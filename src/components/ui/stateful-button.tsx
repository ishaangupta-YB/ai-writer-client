...
export const Button = ({ className, children, disabled, ...props }: ButtonProps) => {
  ...
  return (
    <motion.button
      ...
      ref={scope}
      className={cn(
        "brutal-btn flex min-w-[120px] cursor-pointer items-center justify-center gap-2 border-2 border-border bg-primary px-6 py-2 font-bold uppercase text-primary-foreground shadow-brutal transition-all duration-100 hover:-translate-y-0.5 hover:bg-primary/90 active:translate-y-0 active:shadow-none disabled:pointer-events-none disabled:opacity-50",
        className,
      )}
      disabled={disabled}
      {...buttonProps}
